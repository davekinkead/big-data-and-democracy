...
In contrast, bidirectional communication occurs when nodes can respond to the messages in the same context and medium.  Interpersonal communication is bidirectional, while the characteristics of different forms of technologically mediated communication determine whether it is unidirectional or bidirectional.  The form of technology used to communicate hence affects how political communication takes place and who is communicatively privileged over others in distributing their messages.

The nodes may differ in how many other nodes it is committed with (i.e., with whom an individual may communicate with).  The number of other nodes that any given node may communicate with is the 
